...
class SimulatedAnnealingSVM:
    ...
    def __init__(self, xtrain, ytrain, xtest, num_samples, num_train_samples, num_features, num_qubits_per_a, num_iter):
        ...
        for data in it.product(np.arange(0, self.num_train_samples), repeat=2):
            for qubit in it.product(np.arange(0, self.num_qubits_per_a), repeat=2):
                self.quadratic[("a{}(q{})".format(data[0], qubit[0]), "a{}(q{})".format(data[1], qubit[1]))] = 0.5*(2**(
                    qubit[0]+qubit[1]))*self.ytrain[data[0]]*self.ytrain[data[1]]*kernel(self.xtrain[data[0]], self.xtrain[data[1]], 16)
        ...
        self.bqm = dimod.BinaryQuadraticModel(
            self.linear, self.quadratic, 0, 'BINARY')
    ...
    def check_constraint(self, sample):
        ...
        for data in range(0, self.num_train_samples):
            ...
            for qubit in range(0, self.num_qubits_per_a):
                lagrange_multipliers["a{}".format(
                    data)] += self.sampleset_iterator[sample]["a{}(q{})".format(data, qubit)] * (2*qubit)

        # Calculation of the summation(an*yn)
        sum_lagrange_target = 0
        for data in range(0, self.num_train_samples):
            sum_lagrange_target += lagrange_multipliers["a{}".format(
    ...
    def mean_samples(self, sampleset, max_samples):
        ...
        slice_unique_samples = unique_samples.iloc[:max_samples,
                                                   :self.num_samples*self.num_qubits_per_a]
        ...
        for data in range(0, self.num_train_samples):
            ...
            for qubit in range(0, self.num_qubits_per_a):
                lagrange_multipliers["a{}".format(
                    data)] += mean_lagrange_multipliers["a{}(q{})".format(data, qubit)] * (2**qubit)
    ...
    def get_weights(self, alphas):
        ...
        for feature in range(0, self.num_features):
            ...
            for data in range(0, self.num_train_samples):
                weights["w{}".format(feature)] += alphas["a{}".format(data)] * \
                    self.ytrain[data]*self.xtrain[data][feature]
    ...
    def run_simulated_anneal(self):
        ...
        while not valid and count < len(self.sampleset_iterator):
            valid, self.valid_lagrange_multipliers = self.check_constraint(
                count)
            count += 1
        if valid:
            self.valid_weights = self.get_weights(
                self.valid_lagrange_multipliers)
        # If there isn't any sample fulfilling the constraint, calculate the mean of the distinct samples
            ...
            self.valid_lagrange_multipliers = self.mean_samples(
                self.sampleset, max_samples)
            self.valid_weights = self.get_weights(
                self.valid_lagrange_multipliers)
    ...
    def get_yscore(self):
        ...
        for testpoint in range(0, self.num_samples-self.num_train_samples):
            ...
            for feature in range(0, self.num_features):
                self.yscore[testpoint] += self.valid_weights["w{}".format(
                    feature)]*self.xtest[testpoint][feature]
        return self.yscore
...
class DwaveSVM:
    def __init__(self, xtrain, ytrain, xtest, num_samples, num_train_samples, num_features, num_qubits_per_a, num_iter, num_experiments, num_samples_per_split):
        ...
        self.num_splits = int(self.num_train_samples /
                              self.num_samples_per_split)
    ...
    def mean_samples(self, sampleset, split, max_samples):
        ...
        # Take only max_samples samples
        slice_unique_samples = unique_samples.iloc[:max_samples,
                                                   :self.num_samples_per_split*self.num_qubits_per_a]
        # Calculate the mean of those samples
        mean_lagrange_multipliers = slice_unique_samples.mean()
    ...
    def predict(self, weights):
        ...
        for testpoint in range(0, self.num_samples-self.num_train_samples):
            yscore.append(0)
            for feature in range(0, self.num_features):
                yscore[testpoint] += weights["w{}".format(
                    feature)]*self.xtest[testpoint][feature]
    ...
    def run_dwave(self):
        ...
        for split in range(0, self.num_experiments):
            ...
            for data in it.product(self.data_split[split], repeat=2):
                for qubit in it.product(np.arange(0, self.num_qubits_per_a), repeat=2):
                    self.quadratic[("a{}(q{})".format(data[0], qubit[0]), "a{}(q{})".format(data[1], qubit[1]))] = float(
                        0.5*(2**(qubit[0]+qubit[1]))*self.ytrain[data[0]]*self.ytrain[data[1]]*kernel(self.xtrain[data[0]], self.xtrain[data[1]], 16))
            # Definition of the linear coefficients, for each qubit an(qi), as -B^i
            # This corresponds to the second term of the classical svm minimisation problem -summation(an)
            self.linear = {}
            for data in self.data_split[split]:
            ...
            self.sampleset = self.sampler.sample(
                self.bqm, num_reads=self.num_iter)
            ...
            if valid:
                self.valid_weights = self.get_weights(
                    self.valid_lagrange_multipliers, split)
                self.predictions["exp{}".format(split)], self.yscore["exp{}".format(
                    split)] = self.predict(self.valid_weights)
            # If there isn't any sample fulfilling the constraint, calculate the mean of the distinct samples
            else:
                print("Sample fulfilling constraint not found")
                self.valid_lagrange_multipliers = self.mean_samples(
                    self.sampleset, split, 10)
                self.valid_weights = self.get_weights(
                    self.valid_lagrange_multipliers, split)
                self.predictions["exp{}".format(split)], self.yscore["exp{}".format(
                    split)] = self.predict(self.valid_weights)
...
classic_precision, classic_recall, classic_f1, classic_accuracy = evaluate(
    classic_predictions)
eval_roc_curve(classic_yscore, 'Classic ROC and AUC')
save_files(path, 'classic-results.txt', classic_precision,
           classic_recall, classic_f1, classic_accuracy)

sim_model = SimulatedAnnealingSVM(
    xtrain, ytrain, xtest, num_samples, num_train_samples, num_features, 2, 1000)
...
save_files(path, 'sim-results.txt', sim_precision,

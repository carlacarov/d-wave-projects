    ...
    for sq in range(number_of_squares):
        for pos_1 in range(number_of_squares):
            ...
            for pos_2 in range(pos_1+1, number_of_squares):
                Q[(get_movement_label(sq, pos_1),
                   get_movement_label(sq, pos_2))] += penalty
    ...
    for x, y in itertools.combinations(range(number_of_squares), 2):
        for pos in range(number_of_squares-1):
            ...
            if (not valid_move(x, y)):
                Q[(get_movement_label(x, pos),
                   get_movement_label(y, nextpos))] += 2*penalty
                Q[(get_movement_label(y, pos),
                   get_movement_label(x, nextpos))] += 2*penalty
    return Q
...
def show_solution_board(solution):
    ...
    for el in solution:
        if (solution[el] == 1):
            ...
            ax.text(row, col, square_position, ha='center', va='center', bbox=dict(
